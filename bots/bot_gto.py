...
def analyze_opponent(history, my_id):
    ...
    avg_bet_size = sum(bet_sizes) / len(bet_sizes) if bet_sizes else 100
    
    is_nit = tightness > 0.55
    is_loose = tightness < 0.25
    is_aggressive = aggression > 0.4
    is_passive = aggression < 0.15
    
    return {
        'aggression': aggression,
        'tightness': tightness,
        'frequency': frequency,
        'bet_size': avg_bet_size / 200,
        'is_nit': is_nit,
        'is_loose': is_loose,
...
def calculate_implied_odds(my_chips, amount_to_call, pot_size, hand_strength):
    ...
    return base_implied * (1 + stack_ratio * 0.6)

def gto_bluff_frequency(pot_size, stack_size, hand_strength, opponent_profile):
    pot_odds = stack_size / (pot_size + stack_size)
    
    base_freq = 0.0
    if hand_strength < 25:
        base_freq = max(0.08, 0.35 - pot_odds)
    elif hand_strength < 45:
        base_freq = max(0.03, 0.18 - pot_odds * 0.6)
...
def calculate_cbet_size(pot_size, hand_strength, position_advantage):
    if hand_strength >= 90:
        return min(pot_size * 0.8, pot_size + 100)
...
def get_action():
    ...
    if round_num == 0:
        ...
        if adjusted_strength >= 95:
            if amount_to_call == 0:
                raise_to = max(200, int(effective_stack * 0.05))
            elif amount_to_call <= 100:
                raise_to = max(350, int(effective_stack * 0.08))
            elif amount_to_call <= 300:
                if adjusted_strength >= 98:
                    raise_to = int(effective_stack * 0.12)
                else:
            ...
            if raise_to > 0 and my_chips > amount_to_call:
                return min(raise_to, my_chips - amount_to_call)
            return amount_to_call if my_chips >= amount_to_call else -2
            ...
            elif amount_to_call <= 400 and adjusted_strength >= 88:
                return amount_to_call
            else:
                return -1
            ...
            if amount_to_call == 0:
                if position_factor > 1.1 and hand_strength >= 45:
                    return 0
                return 0
            else:
                return -1
        ...
        if opponent_profile['is_aggressive'] and made_strength >= 75:
            adjusted_strength *= 1.12
        if opponent_profile['is_passive'] and made_strength >= 60:
        ...
        bluff_freq = gto_bluff_frequency(pot_size, effective_stack, adjusted_strength, opponent_profile)
        ...
        if adjusted_strength >= 92:
            if amount_to_call > 0:
                raise_amount = min(
                    ...
                    max(100, int(pot_size * 0.85), int(amount_to_call * 2.8))
                )
                if raise_amount > 0:
                    return raise_amount
                return -2
            else:
                return min(my_chips, max(120, int(pot_size * 0.75)))
        
        elif adjusted_strength >= 82:
            if amount_to_call > 0:
                if amount_to_call <= pot_size * 0.12:
                    return amount_to_call
                elif amount_to_call <= pot_size * 0.3 and made_strength >= 88:
                    return amount_to_call
                else:
                    raise_amount = min(
                        my_chips - amount_to_call,
                        max(80, int(pot_size * 0.55), int(amount_to_call * 2.2))
                    )
                    if raise_amount > 0:
                        return raise_amount
                    return amount_to_call
            else:
                cbet_size = calculate_cbet_size(pot_size, adjusted_strength, position_factor)
                return min(my_chips, int(cbet_size))
        
        elif adjusted_strength >= 70:
            if amount_to_call > 0:
                if effective_odds <= 0.18:
                    return amount_to_call
                elif effective_odds <= 0.3 and made_strength >= 65:
                    return amount_to_call
                else:
                    return -1
            else:
                if made_strength >= 75:
                    return min(my_chips, int(pot_size * 0.3))
                return 0
        
        elif adjusted_strength >= 52:
            if amount_to_call > 0:
                if effective_odds <= 0.1 and (made_strength >= 58 or draw_odds > 0.28):
                    return amount_to_call
                else:
                    return -1
            else:
                if draw_odds > 0.32 and not opponent_profile['is_aggressive']:
                    bluff_amount = min(my_chips, int(pot_size * 0.25))
                    if bluff_amount > 0:
                        return bluff_amount
                return 0
        
        elif adjusted_strength >= 38:
            if amount_to_call == 0:
                if draw_odds > 0.38 and not opponent_profile['is_aggressive']:
                    bluff_amount = min(my_chips, int(pot_size * 0.18))
                    if bluff_amount > 0:
                        return bluff_amount
                return 0
            else:
            ...
            if amount_to_call == 0 and bluff_freq > 0.12 and not opponent_profile['is_nit']:
                bluff_amount = min(my_chips, int(pot_size * 0.12))
                if bluff_amount > 0:
                    return bluff_amount
            return -1
